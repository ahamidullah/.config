...
syn match cType   "\v(struct\s+)@<=[a-zA-Z0-9_]+\s*$"
syn match cTypeInst   "^}\s*\h(\w*)\s*;"
syn match cType   "\v(enum\s+)@<=[a-zA-Z0-9_]+"

syntax match  cTypeInst "}\s*\h\w*\s*;"hs=s+1,he=e-1

syntax region  cFuncLine oneline start="^\h" matchgroup=cFuncProto end="\h\w*\s*("he=e-1

syntax match cFuncDef "^\h\w*\s*("he=e-1

syntax match cFuncName contained "\s\+\w\+\s*("he=e-1
syntax match cFuncDef "^\h.\+\s\+\w\+\s*(.*)\s*$" contains=cFuncName
...
if !exists("did_c_syntax_inits")
  ...
  highlight link cTypeDefStruct        Defined
